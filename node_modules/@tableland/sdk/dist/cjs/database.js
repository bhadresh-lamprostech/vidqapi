...
class Database {
     ...
     */
    static readOnly(chainNameOrId) {
        console.warn("`Database.readOnly()` is a depricated method, use `new Database()`");
     ...
     */
    static async forSigner(signer) {
        const baseUrl = await (0, index_js_1.extractBaseUrl)({ signer });
     ...
     */
    prepare(sql) {
        return new statement_js_1.Statement(this.config, sql);
    }
    /**
    ...
    // everything else a single result wrapped in an Array for backward compatability.
        ...
        try {
            ...
            const normalized = await Promise.all(statements.map(async (stmt) => await (0, index_js_1.normalize)(stmt.toString())));
            const type = normalized
                .map((stmt) => stmt.type)
                .reduce((a, b) => (a === b ? a : null));
            if (type == null) {
                throw new Error("statement error: batch must contain uniform types (i.e. one of: create, write, read, acl)");
            }
            // "read" statement types are the simple case, we just do each of the queries
            // and return an Array of the query results.
            if (type === "read") {
                return await Promise.all(statements.map(async (stmt) => await stmt.all(undefined, opts)));
            }
            // For "create" statement types, each statement must be a single create sql query
            if (type === "create") {
                const receipt = await (0, index_js_1.checkWait)(this.config, await (0, lowlevel_js_1.execCreateMany)(this.config, statements.map((stmt) => stmt.toString())));
            ...
            const runnables = (await Promise.all(normalized.map(async function (norm) {
                return await normalizedToRunnables(norm);
            }))).flat();
            const receipt = await (0, index_js_1.checkWait)(this.config, await (0, lowlevel_js_1.execMutateMany)(this.config, runnables));
            // TODO: wrapping in an Array is required for back compat, consider changing this for next major
            return [(0, utils_js_1.wrapResult)(receipt, performance.now() - start)];
        }
        catch (cause) {
            if (cause.message.startsWith("ALL_ERROR") === true) {
                throw (0, lowlevel_js_1.errorWithCause)("BATCH_ERROR", cause.cause);
            }
            throw (0, lowlevel_js_1.errorWithCause)("BATCH_ERROR", cause);
        }
    }
    /**
     ...
     */
    async exec(statementStrings, opts = {}) {
        // TODO: Note that this method appears to be the wrong return type in practice.
        try {
            const { statements } = await (0, index_js_1.normalize)(statementStrings);
            ...
            if (cause.message.startsWith("RUN_ERROR") === true) {
                throw (0, lowlevel_js_1.errorWithCause)("EXEC_ERROR", cause.cause);
            }
            throw (0, lowlevel_js_1.errorWithCause)("EXEC_ERROR", cause);
        }
    }
    /**
     ...
     */
    async dump(_opts = {}) {
        throw (0, lowlevel_js_1.errorWithCause)("DUMP_ERROR", new Error("not implemented yet"));
    }
}
 ...
 */
async function normalizedToRunnables(normalized) {
    if (normalized.type !== "write" && normalized.type !== "acl") {
        throw new Error("converting to runnable is only possible for mutate statements");
    }
    if (normalized.tables.length > 1) {
        throw new Error("each statement can only touch one table. try batching statements based on the table they mutate.");
    }
    const { tableId } = await (0, index_js_1.validateTableName)(normalized.tables[0]);
    return [
        {
            tableId,
